function m_air = FinalScheme(Nx,Ny,X,Y,Nt,T,p,Ux,Uy,D,S)
    ...
    for j=1:p
        ...
        for n=1:Nt
                ...
                m_air(2:Nx+1,2:Ny+1,j)=max(0,m_air(2:Nx+1,2:Ny+1,j)+dt*(D*laplacien-Ux(:,:,n).*derivee_x - Uy(:,:,n).*derivee_y+S(:,:,j)));
